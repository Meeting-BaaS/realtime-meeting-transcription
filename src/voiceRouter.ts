...
class VoiceRouterClient {
  ...
  constructor(provider?: TranscriptionProvider) {
    this.provider = (provider || transcriptionConfig.defaultProvider) as TranscriptionProvider;
    ...
    if (!providers[this.provider]) {
      ...
      this.provider = Object.keys(providers)[0] as TranscriptionProvider;
    }
  ...
  async initSession(): Promise<boolean> {
    try {
      ...
      this.streamingSession = await this.router.transcribeStream(
        ...
        {
          onTranscript: (event) => {
            if (this.onTranscriptionCallback && event.text) {
              ...
              this.onTranscriptionCallback(event.text, isFinal);
            }
          },
          onError: (error) => {
